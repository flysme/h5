<!-- 浮动在索引图上的容器 -->
<template>
  <div v-if="VIEWS[viewId].indexpic_float_widget" :class="['indexpic_float_container',VIEWS[ VIEWS[viewId].indexpic_float_widget ].view, `orientation_${VIEWS[ VIEWS[viewId].indexpic_float_widget ].orientation}`]" :style="getStyle(VIEWS[VIEWS[viewId].indexpic_float_widget])[VIEWS[VIEWS[viewId].indexpic_float_widget].view]" >
    <!-- 引入扩展区域 -->
    <extend :key="idx" :data="{VIEWS: VIEWS, subviews:item, data: dataItem,currentDetail:currentDetail}" v-for="(item,idx) in VIEWS[ VIEWS[viewId].indexpic_float_widget ].subviews"></extend>
  </div>
</template>
<script>

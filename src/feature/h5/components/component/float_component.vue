<!-- 高级布局组件 -->
<template>
    <div :ids="VIEWS[ viewId ].content_layout"  :style="getStyle(VIEWS[ VIEWS[ viewId ].content_layout ])[ VIEWS[ VIEWS[ viewId ].content_layout ].view ]"        :class="['relative','float_component',VIEWS[VIEWS[ viewId ].content_layout].view,`float_layout_${VIEWS[VIEWS[ viewId ].content_layout].layout_gravity}`]">
        <!-- 引入平铺 -->
        ...
        <!-- 引入浮动 -->
        <div :class="['tile_container',VIEWS[VIEWS[ viewId ].content_layout].view]" v-if="VIEWS[VIEWS[ viewId ].content_layout].float_views && VIEWS[VIEWS[ viewId ].content_layout].float_views.length > 0">
